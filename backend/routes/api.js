...
router.get("/candidatos/presidenciales", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.query(`
      select c.ID_Candidato,c.Nombre,p.Nombre as Partido,cp.Nombre as Corriente, te.Nombre as Eleccion, cf.url as foto
      from candidatos c
      ...
      join candidatosFoto cf on cf.ID_Candidato = c.ID_Candidato
...
router.get("/candidatos/congreso", async (req, res) => {
  ...
  try {
    ...
    connection = await getConnection();
    const query = `
    SELECT c.ID_Candidato, c.Nombre, p.Nombre AS Partido, cp.Nombre AS Corriente,te.Nombre as Eleccion, cir.Nombre AS Circunscripcion,cf.url as foto
    FROM Candidatos c
    JOIN PartidosPoliticos p ON c.ID_Partido = p.ID_Partido
    JOIN CorrientesPoliticas cp ON p.ID_Corriente = cp.ID_Corriente
    ...
    JOIN candidatosFoto cf on cf.ID_Candidato = c.ID_Candidato
      WHERE te.Nombre = ? ${
        idCircunscripcion ? "AND c.ID_Circunscripcion = ?" : ""
      }
    `;
    const params = idCircunscripcion
      ? [tipoEleccion, idCircunscripcion]
      : [tipoEleccion];
...
router.get("/candidatos/:id", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await connection.query(
      `
      select c.Nombre,p.Nombre as Partido,cp.Nombre as Corriente,te.Nombre as Eleccion, ll.InformacionAdicional,cf.url as foto
      from candidatos c
      join partidospoliticos p on c.ID_Partido = p.ID_Partido
      join corrientespoliticas cp on p.ID_Corriente  =cp.ID_Corriente
